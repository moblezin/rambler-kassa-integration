        ...
        [1, 2, 3].forEach(function (index) {
            ...
            request.get(config.linkMovies.replace('{{num}}', String(index)), function (err, res, body) {
                if (err) {
                    ...
                    xmlParser.parseString(body, function (err:Object, body:MoviesXML) {
...
function fetchSessions() {
    return Q.Promise(function (resolve, reject, notify) {
        request(config.linkSchedule, function (err, res, body:string) {
 ...
 */
function parseSessions(sessions:Sessions) {
    var master:string = '';
    var num:number = 1;
    var sortedKeys:Array<string> = Object.keys(sessions).sort();
    ...
    for (var key in sortedKeys) {
        if (sessions.hasOwnProperty(sortedKeys[key])) {
            ...
            if (movie.sessions.length <= 10) {
                master += config.csv.replace('{{num}}', String(num))
                    .replace('{{name}}', movie.name)
                    .replace('{{format}}', movie.format)
                    .replace('{{ageRestriction}}', movie.age)
                    .replace('{{sessions}}', movie.sessions.join(';'))
                    .replace('{{prices}}', movie.prices.join(';'))
                    .replace('{{halls}}', movie.halls.join(';'))
                ;
            } else {
                master += config.csv.replace('{{num}}', String(num))
                        .replace('{{name}}', movie.name)
                        .replace('{{format}}', movie.format)
                        .replace('{{ageRestriction}}', movie.age)
                        .replace('{{sessions}}', movie.sessions.splice(0, 10).join(';'))
                        .replace('{{prices}}', movie.prices.splice(0, 10).join(';'))
                        .replace('{{halls}}', movie.halls.splice(0, 10).join(';'))
                    + config.csv.replace('{{num}}', String(num))
                        .replace('{{name}}', movie.name)
                        .replace('{{format}}', movie.format)
                        .replace('{{ageRestriction}}', movie.age)
                        .replace('{{sessions}}', movie.sessions.join(';'))
    ...
    function sortSessions(sessionTimes) {
        ...
        var sessionStrings:Array<string> = [];
        ...
        dateArray.forEach(function (date:Date) {
...
function writeOutput(master:string) {
    try {
...
function getConfig() {
    var data:Config = JSON.parse(iconv.decode(fs.readFileSync(path.resolve(folder, 'config.json')), 'win1251'));
    var date:Date = new Date();
    ...
    data.linkSchedule = data.linkSchedule
        .replace('{{key}}', data.key)
        .replace('{{objectId}}', String(data.objectId))
        .replace('{{dateFrom}}', data.datefrom)
        .replace('{{dateTo}}', data.dateto)
        .replace('{{cityId}}', data.cityid)
        .replace('{{salesupportedonly}}', data.salesupportedonly);
    return data;
}
